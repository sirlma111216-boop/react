...
import { AssetImage, Formula } from './common';
...
export function ContractCard({ c, team, round, mode, actions }: { c: ContractInstance; team: TeamState | null; round: number; mode: 'offer' | 'held' | 'auction' | 'plain'; actions?: React.ReactNode }) {
  ...
  return (
    <div className={`ccard ${mode === 'held' ? 'held' : ''} ${c.special ? 'special' : ''}`}>
      ...
      <div>
        <div className="row-between"><span className="ctitle">{c.title}</span><span className="reward">+{c.reward}</span></div>
        ...
        <div className="creq">
          {c.requirements.map((r, i) => {
            ...
            return <span key={i} className={`tag ${team && have >= r.units ? 'tag-teal' : ''}`} title={`${MATERIALS[r.materialId]!.displayName} · 이력: ${r.tags.map(tagLabel).join('/')}`}><Formula id={r.materialId} withPhase={false} /> {team ? `${have}/` : ''}{r.units}칸 <span style={{ opacity: 0.7 }}>{r.tags.map(tagLabel).join('/')}</span></span>;
          })}
        </div>
        <div className="row-between" style={{ marginTop: 4 }}>
          <span className={`deadline ${left <= 1 && mode === 'held' ? 'urgent' : ''}`}>{c.special ? '경기 종료까지' : mode === 'held' ? `기한: ${c.deadlineRound}R (${left}라운드 남음)` : `기한 ${c.deadlineRound}R`}{c.bidPaid ? ` · 낙찰 ${c.bidPaid}코인` : ''}</span>
          {actions}
        </div>
        {mode === 'held' && !sat.ok && sat.missing.length > 0 && <div className="muted small">부족: {sat.missing.join(', ')}</div>}
      </div>
    </div>
  );
}
