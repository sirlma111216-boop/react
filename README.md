...
- 라운드 = 상의 시간 30초(주문 받기·입찰·의논) → 행동 시간 90초(차례인 사람이 행동 3번) → 마무리 30초(완성품 도착·인계). 매 라운드 차례가 팀 안에서 돌아갑니다. 차례가 아니면 카드를 살펴보고 👍 추천합니다.
- 만들기 카드를 눌러 **만들기 ×1/×2**. 카드의 "왜 이만큼 필요할까?"에서 원자 수·질량·기체 부피비를 봅니다.
- 하단 창고를 눌러 **정리하기**(응축하기·거르기·결정 만들기·정제하기, 기다림 없이 즉시). 섞여 나온 것은 정리해야 배달할 수 있고, 가게에서 산 재료는 그대로 배달할 수 없습니다.
- 좌측 주문 카드에서 **받기**(행동 소모 없음), 조건이 채워지면 **배달**. 3·6라운드에는 도시 특별 주문에 0~6코인 비공개 입찰.
- 휴대전화에서는 하단 탭(공방/주문/창고/팀)으로 같은 기능을 씁니다. 보드 상단 💡 안내가 지금 할 일을 알려줍니다.
